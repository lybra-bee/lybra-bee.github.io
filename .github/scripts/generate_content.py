...
def generate_image(title, slug):
    """Генерация изображения через SubNP с SSE"""
    try:
        logging.info("[SubNP] Запрос на генерацию изображения...")
        payload = {"prompt": title, "model": SUBNP_MODEL}
        headers = {"Content-Type": "application/json"}
        r = requests.post(f"{SUBNP_BASE_URL}/api/free/generate", headers=headers, json=payload, stream=True, timeout=60)
        ...
        for line in r.iter_lines(decode_unicode=True):
            ...
            try:
                ...
                if status == "complete" and "imageUrl" in data:
                    image_url = data["imageUrl"]
                    break
                elif status == "error":
                    logging.error(f"❌ SubNP ошибка: {data.get('message')}")
                    break
            except Exception as e:
                continue
