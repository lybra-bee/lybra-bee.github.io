...
def generate_article_title():
    prompt = "Придумай ёмкий заголовок статьи про ИИ и высокие технологии. Например, применение ИИ в медицине, строительстве, новые модели генерации или уроки по нейросетям."
    # OpenRouter
    try:
...
def generate_article_body(title):
    prompt = f"Напиши развернутую статью по заголовку: '{title}' на русском языке, структурированно и с подзаголовками."
    try:
        response = requests.post(
            "https://api.openrouter.ai/v1/chat/completions",
            headers={"Authorization": f"Bearer {os.environ['OPENROUTER_API_KEY']}"},
...
# --- Копируем изображения в assets/gallery ---
