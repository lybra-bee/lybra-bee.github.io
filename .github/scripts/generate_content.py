...
def generate_article():
    header_prompt = "Проанализируй последние тренды в нейросетях и высоких технологиях и придумай заголовок до восьми слов"
    try:
...
def generate_image(title, slug):
    if not CLIPDROP_API_KEY:
        logging.warning("⚠️ CLIPDROP_API_KEY не найден, используем PLACEHOLDER")
        return PLACEHOLDER
    try:
        logging.info("🖼 [Clipdrop] Запрос на генерацию изображения...")
        headers = {"x-api-key": CLIPDROP_API_KEY, "Content-Type": "application/json"}
        payload = {"prompt": title, "model": "stable-diffusion"}
        r = requests.post("https://clipdrop-api.co/text-to-image/v1", headers=headers, json=payload)
        r.raise_for_status()
        img_data = r.content
...
def update_gallery(title, slug, image_path):
    ...
    gallery.insert(0, {"title": safe_yaml_value(title), "alt": safe_yaml_value(title), "src": image_path if image_path.startswith('/') else f'/{image_path}'})
    gallery = gallery[:20]
