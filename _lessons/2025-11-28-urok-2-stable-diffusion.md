...
Указывай качество: «8k», «ультра детализация», «masterpiece»
Используй negative prompt (что НЕ должно быть)
Указывай освещение: «мягкий свет заката», «неон ночью»
Добавляй эмоции: «грустная», «восторженная», «мечтательная»
Экспериментируй с соотношением сторон: --ar 16:9 в некоторых веб-версиях
Указывай ракурс: «вид сзади», «портрет крупным планом»
Добавляй детали окружения: «в библиотеке древних книг», «на Марсе»
...
Одно из них должно быть максимально безумным (пример: «кот в костюме космонавта пьёт кофе на Луне, ретро-футуризм 60-х»)
Три лучших сохрани в хорошем качестве
Выложи в сторис или пост с хештегом #МойПервыйStableDiffusion
(По желанию) добавь в описание, какой промпт использовал
...
Персональный промпт от меня для твоего стиля
�


Что будет дальше
Понедельник, 1 декабря — Лайфхак-понедельник: как ускорить генерацию в 5–10 раз и сэкономить GPU
Пятница, 5 декабря — Урок 3: ControlNet — рисуем по своим фотографиям, скетчам и позам
Декабрь — LoRA-обучение, свои стили, анимация, 3D
�
...
Через год ты посмотришь на свои первые картинки и улыбнёшься:
