...
DEEPAI_API_KEY = os.getenv("DEEPAI_API_KEY")  # ← Обязательно добавить в GitHub Secrets!
...
# -------------------- Изображение: DeepAI --------------------
def generate_image_deepai(title):
    if not DEEPAI_API_KEY:
        logging.warning("DEEPAI_API_KEY отсутствует — пропуск генерации изображения")
        return None

    url = "https://api.deepai.org/api/text2img"
    headers = {"api-key": DEEPAI_API_KEY}

    prompt = f"{title}, futuristic artificial intelligence, photorealistic, high detail, professional photography, 8k, cinematic lighting, vibrant colors"

    payload = {"text": prompt}

    for attempt in range(5):
        try:
            logging.info(f"Генерация изображения через DeepAI (попытка {attempt+1})...")
            r = requests.post(url, headers=headers, data=payload, timeout=90)
            
            if r.status_code == 429:
                logging.warning("Лимит DeepAI исчерпан (429 Too Many Requests)")
                return None
            if not r.ok:
                logging.warning(f"DeepAI ошибка {r.status_code}: {r.text}")
                time.sleep(5)
                continue
            ...
            if not image_url:
                logging.warning("DeepAI не вернул URL изображения")
                continue

            # Скачиваем изображение
            img_data = requests.get(image_url, timeout=60)
            if not img_data.ok:
                logging.warning("Не удалось скачать изображение от DeepAI")
                continue

            timestamp = int(time.time())
            img_path = IMAGES_DIR / f"deepai-{timestamp}.jpg"
            ...
            logging.warning(f"Ошибка DeepAI: {e}")
            time.sleep(10)
...
def generate_image(title):
    img = generate_image_deepai(title)
    if img:
        return img
    logging.warning("DeepAI не сработал → используем fallback")
