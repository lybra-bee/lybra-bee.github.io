...
CLIPDROP_API_KEY = os.getenv("CLIPDROP_API_KEY")  # оставляем на случай будущего использования
...
def generate_image_kandinsky(prompt, timeout=600):
    ...
    try:
        ...
        if not r.ok:
            ...
            return None
        models = r.json()
        model_id = models[0]["id"]  # Обычно Kandinsky 3.1
...
# -------------------- ClipDrop (оставляем как резерв, но не используем в цепочке) --------------------
def generate_image_clipdrop(prompt):
    if not CLIPDROP_API_KEY:
        logging.warning("ClipDrop key absent, skipping")
        return None
    ...
    try:
        ...
        with open(img_path, "wb") as f:
            f.write(r.content)
        logging.info(f"Image generated by ClipDrop: {img_path}")
        return str(img_path)
    except Exception as e:
...
def generate_image(prompt):
    ...
    if img:
        return img
    # Если захотите добавить ClipDrop обратно — раскомментируйте:
